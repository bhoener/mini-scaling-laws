...
def matmul_kernel(
    ...
    # this is not correct. we can't just divide by GROUP_SIZE ** 2 because the size of groups is not fixed
    # [ 0  1 | 2  3 | 4 ] -> [ 0  1 | 4  5 | 8 ]
    # [ 5  6 | 7  8 | 9 ]    [ 2  3 | 6  7 | 9 ]
    # ------- ------ ----    ------- ------ ---
    # [10 11 | 12 13| 14]    [10 11 | 12 13| 14]
    pids_per_group_row = GROUP_SIZE * blocks_N

    group_row = pid // pids_per_group_row
    group_row_start = group_row * pids_per_group_row
    group_size_adj_m = min(GROUP_SIZE, blocks_M - group_row * GROUP_SIZE)
    group_col = (pid - group_row_start) // (GROUP_SIZE * group_size_adj_m)

    group_start = group_row_start + group_col * GROUP_SIZE * group_size_adj_m

    group_size_adj_n = min(GROUP_SIZE, blocks_N - GROUP_SIZE * group_col)
    pid_m = group_row * GROUP_SIZE + (pid - group_start) // group_size_adj_n
    # subtract start of group to get id relative to group
    # mod by group size to get the n component and then add back group M
    pid_n = group_col * GROUP_SIZE + (pid - group_start) % group_size_adj_n
...
def test_matmul(
    M: int, K: int, N: int, atol=1e-2, rtol=1e-1, device=DEVICE, dtype=torch.float16
) -> None:
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="TFLOPS",
        styles=[("green", "-"), ("blue", "-")],
        plot_name="memory-matmul-torch-triton",
    )
)
def benchmark(M: int, N: int, K: int, provider: str) -> tuple[float, ...]:
    ...
    tflops = lambda ms: 3 * M * N * K / 1e12 / (ms * 1e-3)
